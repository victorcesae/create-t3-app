...
<div data-components="betterauth">

#### `src/app/api/auth/[...all]/route.ts`

The `[...all]/route.ts` file is the BtterAuth authentication slug route. It is used to handle authentication requests. See [BetterAuth usage](usage/better-auth) for more information on BetterAuth, and [Next.js Dynamic Routes Docs](https://nextjs.org/docs/app/building-your-application/routing/dynamic-routes) for info on catch-all/slug routes.
...
<div data-components="betterauth+trpc">

#### `src/server/auth.ts`

The main entrypoint for server-side authentication logic. Here, we setup the BetterAuth [configuration options](usage/better-auth), provide some DX utilities for authentication such as retrieving the user's session on the server-side. See [BetterAuth usage](usage/better-auth#usage-with-trpc) for more information.
